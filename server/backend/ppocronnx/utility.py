...
def padding_img(img, input_size=[640, 640]):
    padding_im_h = input_size[0]
    padding_im_w = input_size[1]
    im_c = img.shape[2]
    im_h = img.shape[0]
    im_w = img.shape[1]
    padding_im = np.zeros((padding_im_h, padding_im_w, im_c), dtype=np.float32) + 255
...
def draw_ocr_box_result(img, boxed_result, drop_score=0.5, font_path="C:/Windows/Fonts/simfang.ttf"):
